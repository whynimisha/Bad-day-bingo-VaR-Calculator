...
DEFAULT_WINDOW = 750       # trading days for vol/corr estimation (~1y)
...
def simulate_pnl_normal(positions: List[Position], prices_now: pd.Series,
    ...
    # Deduct 0.15% transaction cost from total portfolio value
    total_value = sum(p.qty * prices_now[p.symbol] * p.multiplier for p in positions)
    pnl = pnl - (0.0015 * total_value)
...
def simulate_pnl_historical(positions, prices_now, rets_df, scenarios: int, horizon: int = 1) -> np.ndarray:
...
def fetch_price_data(tickers, window_days, extra_buffer_days=40):
    series = []
    cal = mcal.get_calendar("XNSE")   # NSE calendar
    ...
    # Align with NSE trading sessions
    schedule = cal.schedule(start_date=prices.index.min(), end_date=prices.index.max())
    valid_dates = schedule.index.normalize()
    prices = prices.loc[prices.index.isin(valid_dates)]
    print(f"Valid dates: {len(valid_dates)}")
    return prices.tail(2 * window_days + extra_buffer_days)
...
def main():
    ...
    if prices.shape[1] < len(symbols):
        print(f"Warning: Only {prices.shape[1]} of {len(symbols)} tickers fetched")

    print(f"Prices length = {len(prices)}, needed >= {2*args.window}")
    returns = prices.pct_change().dropna()
    print(f"[3/6] Computed log returns. Shape={returns.shape}")
   
    nsei = yf.download("^NSEI", period="5y", interval="1d", auto_adjust=True, progress=False)
    nsei_std = nsei["Close"].pct_change().std()
    print("Portfolio avg std:", returns.std().mean(), "NSEI std:", nsei_std)
    ...
    # Volatility calibration floor (1.5%)
    if returns.std().mean() < 0.015:
        factor = 0.015 / returns.std().mean()
        print(f"Scaling Sigma by {factor:.2f} to match NSE vol floor 1.5%")
        Sigma = Sigma * factor

...
def build_args():
    ...
    if kde:
        args.append("--kde")
    drift_map = {"0": "0", "auto": "auto", "custom %/yr": str(drift_val)}
    args += ["--drift", drift_map[drift]]
    if seed > 0: args += ["--seed", str(seed)]
    if liq_bp > 0: args += ["--liq_haircut_bp", str(liq_bp)]
    if ci > 0: args += ["--ci", str(ci)]
    return args
    ...
    if not os.path.exists(ENGINE):
        ...
        if not res:
            ...
            with col1:
                ...
                # Show multi-horizon results
                if "multi_horizon" in res:
                    st.subheader("Multi-horizon")
                    st.dataframe(pd.DataFrame(res["multi_horizon"]))
            with col2:
                st.subheader("Backtest")
                st.json(res.get("backtest", {}))
                if res.get("backtest", {}).get("n_obs", 0) < window:
                    st.warning(
                        f"Backtest observations ({res['backtest']['n_obs']}) < window ({window}). "
